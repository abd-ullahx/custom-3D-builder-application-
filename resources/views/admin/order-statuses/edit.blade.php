...
@section('header', 'Modify Status Stage')

@section('content')
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0 fw-bold">Edit Order Status</h4>
            <small class="text-muted">Modify status name and display badge colors for order processing updates.</small>
        </div>
            ...
            <form action="{{ route('admin.order-statuses.update', $orderStatus->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-select border-start-0 ps-0 bg-light-focus" id="badge_color" name="badge_color" required>
                                <option value="primary" {{ old('badge_color', $orderStatus->badge_color) === 'primary' ? 'selected' : '' }}>Primary Blue</option>
                                <option value="secondary" {{ old('badge_color', $orderStatus->badge_color) === 'secondary' ? 'selected' : '' }}>Secondary Grey</option>
                                <option value="success" {{ old('badge_color', $orderStatus->badge_color) === 'success' ? 'selected' : '' }}>Success Green</option>
                                <option value="danger" {{ old('badge_color', $orderStatus->badge_color) === 'danger' ? 'selected' : '' }}>Danger Red</option>
                                <option value="warning" {{ old('badge_color', $orderStatus->badge_color) === 'warning' ? 'selected' : '' }}>Warning Yellow</option>
                                <option value="info" {{ old('badge_color', $orderStatus->badge_color) === 'info' ? 'selected' : '' }}>Info Cyan</option>
                                <option value="dark" {{ old('badge_color', $orderStatus->badge_color) === 'dark' ? 'selected' : '' }}>Dark Charcoal</option>
                            </select>
                        </div>
                ...
                <div class="mt-5 border-t pt-4">
                    <button type="submit" class="btn btn-primary fw-bold px-4 py-2.5 rounded-3 shadow-sm hover-up">
                        <i class="bi bi-check-circle me-1"></i> Update Status Settings
                    </button>
